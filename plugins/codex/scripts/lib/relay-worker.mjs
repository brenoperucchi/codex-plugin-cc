// relay-worker.mjs — execution side of the MCP transport (TASK-1.3).
//
// Drives queued relay jobs to completion. This is DELIBERATELY separate from the
// MCP coordination facade (relay-mcp-server.mjs): coordination (enqueue/poll) must
...
import { resolveCodexSandboxMode, resolveCodexAutoApprovalPolicy } from "./codex-config.mjs";
...
// Default executor: run a Codex turn via the plugin's app-server (broker, Unix
// socket — NOT the live ws://:4500). runAppServerTurn does not persist a companion
// job (verified: codex.mjs has no state.json writes), so only relay-state.json is
// touched. It has no native AbortSignal, so abort here is best-effort.
async function defaultRunTurn(cwd, { prompt, model, effort, write, onProgress }) {
  const sandbox = write ? (resolveCodexSandboxMode(cwd) ?? "workspace-write") : "read-only";
  const approvalPolicy = write ? resolveCodexAutoApprovalPolicy(cwd) : null;
  ...
  return {
    ok,
    output: result.finalMessage ?? null,
    threadId: result.threadId ?? null,
    touchedFiles: result.touchedFiles ?? [],
    error: result.error?.message ?? (ok ? null : "turn failed")
  };
}
