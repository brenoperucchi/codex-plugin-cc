// relay-jobs.mjs — Durable job-relay for the MCP transport layer (TASK-1.1).
//
...
// (TASK-1.2) and dispatch (TASK-1.3) build on.
//
// WHY A SEPARATE STORE (not state.mjs's state.json):
...
//   from state.mjs: workspace state-dir resolution, id generation, and the per-job
//   event-file location.
//
...
import { resolveStateDir, generateJobId, resolveJobEventFile } from "./state.mjs";
...
// --- interprocess lock (ownership nonce + atomic steal + conditional release) ---
...
function withLock(cwd, fn, { lockTimeoutMs = DEFAULT_LOCK_TIMEOUT_MS, lockStaleMs = DEFAULT_LOCK_STALE_MS } = {}) {
  ensureRelayDir(cwd);
  const lockFile = relayLockFile(cwd);
  ...
  for (;;) {
    try {
      ...
      if (Date.now() - start > lockTimeoutMs) {
        throw new RelayStoreError(`timeout aguardando o lock do relay (${lockFile})`, { code: "LOCK_TIMEOUT" });
      }
      sleepSync(20);
    }
  }
...
function pruneStore(store, now, { retentionMs, maxJobs }) {
  ...
  // 1. Drop terminal jobs older than the retention window (dedup window ends here).
  ...
  // 2. Hard cap: if still over the limit, drop the oldest TERMINAL jobs first
  //    (active work is never dropped).
  if (retained.length > maxJobs) {
    const terminal = retained
      .filter((job) => TERMINAL.has(job.relayState))
      .sort((a, b) => (a.terminalAtMs ?? 0) - (b.terminalAtMs ?? 0));
    const overflow = retained.length - maxJobs;
    const toDrop = new Set(terminal.slice(0, overflow).map((job) => job.id));
...
// --- event log (reuses state.mjs per-job event file) ---------------------
